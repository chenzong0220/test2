...
">
    ...
    ">Financial Dashboard and Program Trading</h2>
</div>
"""
...
# 設定可選擇的日期區間
if choice == choices[0]:
    min_date = datetime.date(2023, 4, 17)
    max_date = datetime.date(2025, 4, 17)
elif choice == choices[1]:
    min_date = datetime.date(2023, 4, 17)
    max_date = datetime.date(2025, 4, 17)
elif choice == choices[2]:
    min_date = datetime.date(2023, 4, 17)
    max_date = datetime.date(2025, 4, 16)
elif choice == choices[3]:
    min_date = datetime.date(2023, 4, 15)
    max_date = datetime.date(2025, 4, 16)
...
end_date = st.date_input("選擇結束日期", value=max_date, min_value=min_date, max_value=max_date)

# 驗證與過濾資料
if start_date > end_date:
    st.error("⚠️ 開始日期不能晚於結束日期")
else:
